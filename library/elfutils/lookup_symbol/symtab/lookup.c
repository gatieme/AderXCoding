...
static int maybe_discarded_sym(const char *name)
{
	 ...
	 * they're not, depending on whether the parent object is table or a
			...
			struct sym_compare_type *child_locals)
{
	...
	for_each_obj_symbol_continue(i, sym, table) {
		...
		printf("[%s %d] find local symbol %s [START]\n", __func__, __LINE__, sym->name);
		found = 0;
		for (child = child_locals; child->name; child++) {
			if (child->type == sym->type &&
			    !strcmp(child->name, sym->name)) {
				found = 1;
				printf("[%s %d] find local symbol %s [END]\n", __func__, __LINE__, sym->name);
				break;
			}
		}
			    ...
			    struct sym_compare_type *child_locals)
{
	...
	for_each_obj_symbol(i, sym, table) {
		...
		if (strcmp(hint, sym->name))
			continue;
		printf("[%s %d] HINT = %s, SYMBOL = %s\n", __func__, __LINE__, hint, sym->name);
		if (!locals_match(table, i, child_locals))
			continue;
		if (table->local_syms)
			printf("find_local_syms for %s: found_dup", hint);

		table->local_syms = sym;
	}

	if (!table->local_syms)
		printf("find_local_syms for %s: couldn't find in table symbol table", hint);
}
...
static void symtab_read(struct lookup_table *table, char *path)
{
	...
	printf("[%s %d] nr = %d\n", __func__, __LINE__, table->obj_nr);
	table->obj_syms = malloc(table->obj_nr * sizeof(*table->obj_syms));
	if (!table->obj_syms)
		printf("malloc table.obj_syms");
	...
	while (fgets(line, 256, file)) {
		...
		if (!strcmp(bind, "LOCAL")) {
			...
			printf("unknown symbol bind %s", bind);
		}
		...
		if (!strcmp(type, "NOTYPE")) {
			...
			printf("unknown symbol type %s", type);
		}

		table->obj_syms[i].name = strdup(name);
		if (!table->obj_syms[i].name)
			printf("strdup");
		printf("[%s %d] SYMBOL = %s\n", __func__, __LINE__, table->obj_syms[i].name);
		i++;
	}
...
int lookup_is_exported_symbol(struct lookup_table *table, char *name)
{
	...
	for_each_exp_symbol(i, sym, table) {
		if (!strcmp(sym->name, name)) {
			if (match)
				printf("duplicate exported symbol found for %s", name);
			match = sym;
		}
	}
...
int main(int argc, char **argv)
{
	...
	table = lookup_open(argv[1], argv[2], argv[3], &locals);

	printf("printk is%s exported\n",
		lookup_is_exported_symbol(table, "__fentry__") ? "" : " not");
	printf("meminfo_proc_show is%s exported\n",
		lookup_is_exported_symbol(table, "meminfo_proc_show") ? "" : " not");

#if 0
	find_this(table, "printk", NULL);
	find_this(table, "pages_to_scan_show", "ksm.c");
	find_this(table, "pages_to_scan_show", "huge_memory.c");
	find_this(table, "pages_to_scan_show", NULL); /* should fail */
#endif
